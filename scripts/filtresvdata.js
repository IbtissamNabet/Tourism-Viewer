...
let newTypes = [];
/* Tableau qui stocke les types décoché dont il faut enlever les marqueurs encore présents */
let oldTypes = [];
...
async function updatePopups() {
    ...
    const repInfos = await fetch('/static/json/data_got.json'); // à changer !
    const infos = await  repInfos.json();
    /* Tableau qui stockera les marqueurs */
    ...
    for(let t = 0; t < infos.length; t++){
        ...
        if(newTypes.includes(nomType)){
            ...
            let marker = L.marker([infos[t].latitude,infos[t].longitude], {title : nomType}).addTo(map);
            ...
            /* On ajoute le marker parmis ceux affichés  */
            markers.push(marker);
        ...
        if(oldTypes.includes(nomType)){
            ...
            for(let m = 0; m < markers.length; m++){
                if(markers[i].options.title === nomType) map.removeLayers(markers[i]);
                /* Le marqueur ne fait plus parti des marqueurs affichés */
                markers = markers.filter((lefts)=> lefts !== markers[i]);
            }
            /* On supprime le type de la liste des markers à enlever */
            oldTypes = oldTypes.filter((lefts)=> lefts !== nomType);
        }
    }
}
...
for (let c = 0; c < sousTypesLieux.length ; c++){
    ...
    check.addEventListener('change', () => {
        ...
        if(check.checked){
            ...
            updatePopups();
            connexion_functions.data_selections(sousTypesSelectionnes);
        }
        else{
...
> à la fin car sinon pas possibles de savoir quels marqueurs supprimer */
